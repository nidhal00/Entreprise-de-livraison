...
MainWindow::MainWindow(QWidget *parent)
    ...
    connect(ui->tricommandes, &QPushButton::clicked, this, &MainWindow::on_triCommandes_clicked);
    connect(ui->pushButton_Appel, &QPushButton::clicked, this, &MainWindow::on_pushButton_Appel_clicked);
    connect(ui->pushButton_SMS, &QPushButton::clicked, this, &MainWindow::on_pushButton_SMS_clicked); // New connection
...
void MainWindow::on_tableView_2_clicked(const QModelIndex &index) {
    if (index.isValid()) {
        ...
        selectedPhoneNumber = model_clients->data(model_clients->index(index.row(), 4)).toString(); // Store phone number
        ui->lineEditTelephone->setText(selectedPhoneNumber);
        ...
        qDebug() << "Client sélectionné avec ID :" << selectedId << "Téléphone :" << selectedPhoneNumber;
    }
}

void MainWindow::on_pushButton_Appel_clicked() {
    if (selectedId == -1) {
        QMessageBox::warning(this, "Erreur", "Veuillez sélectionner un client dans le tableau.");
        return;
    }

    QString phoneNumber = selectedPhoneNumber.trimmed();
    if (phoneNumber.isEmpty() || !QRegularExpression("^[0-9]{8}$").match(phoneNumber).hasMatch()) {
        QMessageBox::warning(this, "Erreur", "Numéro de téléphone invalide ou non disponible.");
        return;
    }

    // Use tel: URI scheme to initiate a call
    QString url = "tel:" + phoneNumber;
    qDebug() << "Tentative d'appel avec le numéro :" << phoneNumber;
    if (!QDesktopServices::openUrl(QUrl(url))) {
        QMessageBox::critical(this, "Erreur", "Impossible d'ouvrir l'application d'appel. Vérifiez que l'application par défaut est configurée.");
        qDebug() << "Échec de l'ouverture de l'URL :" << url;
    } else {
        qDebug() << "Appel initié avec succès pour :" << phoneNumber;
    }
}
...
void MainWindow::on_pushButton_SMS_clicked() {
    ...
    // Query the latest command for this client
    QSqlDatabase db = Connection::getDatabase();
    if (!db.isValid()) {
    ...
    QSqlQuery query(db);
    query.prepare("SELECT STATUT, DATE_COMMANDE FROM COMMANDES WHERE ID_CLIENT = :id_client ORDER BY DATE_COMMANDE DESC FETCH FIRST 1 ROW ONLY");
    query.bindValue(":id_client", selectedId);
    if (!query.exec() || !query.next()) {
        qDebug() << "Aucune commande trouvée pour le client ID :" << selectedId << "ou erreur :" << query.lastError().text();
        // Default message if no command found
        QString defaultMessage = QString("Bonjour %1, ceci est un message de [Votre Société de Livraison]. Contactez-nous pour plus d'infos.").arg(
            ui->lineEditNom->text() + " " + ui->lineEditPrenom->text());
        QString url = QString("sms:%1?body=%2").arg(phoneNumber).arg(defaultMessage.toUtf8().toPercentEncoding());
        if (!QDesktopServices::openUrl(QUrl(url))) {
            QMessageBox::critical(this, "Erreur", "Impossible d'ouvrir l'application SMS. Vérifiez que l'application par défaut est configurée.");
            qDebug() << "Échec de l'ouverture de l'URL SMS :" << url;
        }
        return;
    }
    ...
    QDate dateCommande = query.value(1).toDate();
    QString clientName = ui->lineEditNom->text() + " " + ui->lineEditPrenom->text();
    QString message = QString("Bonjour %1, votre commande du %2 est %3. [Votre Société de Livraison] vous contactera bientôt pour la livraison.")
                          .arg(clientName)
                          .arg(dateCommande.toString("dd/MM/yyyy"))
                          .arg(statut);

    // Limit message to 160 characters (standard SMS limit)
    if (message.length() > 160) {
        message = message.left(157) + "...";
    }

    // Construct SMS URL
    QString url = QString("sms:%1?body=%2").arg(phoneNumber).arg(message.toUtf8().toPercentEncoding());
    qDebug() << "Tentative d'envoi de SMS avec le message :" << message;
    if (!QDesktopServices::openUrl(QUrl(url))) {
        QMessageBox::critical(this, "Erreur", "Impossible d'ouvrir l'application SMS. Vérifiez que l'application par défaut est configurée.");
        qDebug() << "Échec de l'ouverture de l'URL SMS :" << url;
    } else {
        qDebug() << "SMS initié avec succès pour :" << phoneNumber << "avec message :" << message;
    }
}
